import { ThemeConfig, ThemeMode, ColorScheme, ThemeVariables } from '../model/theme.types'
import { THEME_PRESETS } from './theme-presets'
import { loggers } from '@nova/infrastructure/logger'

const STORAGE_KEY = 'syllo-theme-config'
...
export class ThemeService {
  ...
  static applyTheme(config: ThemeConfig): void {
    ...
    if (!preset) {
      loggers.theme.error(`未找到主题预设: ${colorScheme}`)
      return
    }
  ...
  static loadTheme(): ThemeConfig {
    try {
      ...
      loggers.theme.error('加载主题配置失败', { error })
    }
  ...
  static saveTheme(config: ThemeConfig): void {
    try {
      ...
      loggers.theme.error('保存主题配置失败', { error })
    }
  }
